...
export function MacroLineChart({
  ...
  useEffect(() => {
    ...
    const leftTickFormat: (d: number) => string = leftSeries.some(
      (s) => s.formatInMillions
    )
      ? (d) => `${d / 1e6}`
      : d3.format(",");

    const rightTickFormat: ((d: number) => string) | null =
      useDualAxis && yRight
        ? d3.format(",") // or (d) => `${d / 1e6}Mn` if you want millions too
        : null;
    ...
    svg
      ...
      .call(
        d3
          .axisLeft(yLeft)
          .ticks(5)
          .tickFormat(leftTickFormat as any)
      )
      .selectAll<SVGTextElement, number>("text")
      .attr(
        "class",
    ...
    if (useDualAxis && yRight) {
      svg
        ...
        .call(
          d3
            .axisRight(yRight)
            .ticks(5)
            .tickFormat(rightTickFormat as any)
        )
        .selectAll<SVGTextElement, number>("text")
        .attr(
          "class",
      ...
      svg
        ...
        .attr(
          "class",
          "font-sourcecodepro text-slate-600 text-base lg:text-lg font-normal"
        )
